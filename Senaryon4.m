...
vaccinated_col = [];
vaccinated = 0;
vaccination_times = []; % an array to see which person is vaccinated on iteration number
W = 0.8;
tsec = 3;
p = 0.5;
qs = 0.5;
...
while x <= length(vaccinated_col) % movement of the vaccinated
    ...
    z = length(vaccinated_col);
    y = 1;
    while y <= length(vaccination_times) % to check if a person will be vaccinated for the second time
        vaccination_times(y) = vaccination_times(y) + 1;
        if vaccination_times(y) >= tsec
            c = rand();
            if c < W
                fprintf('immunity row and immunity col is %d and %d\n',vaccinated_row(y),vaccinated_col(y));
                immunity_row(end+1) = vaccinated_row(y);
                immunity_col(end+1) = vaccinated_col(y);
                vaccinated_row(y) = [];
                vaccinated_col(y) = [];
                vaccination_times(y) = [];
                y = y - 1;
        ...
        if num ~= 0 % coincided person is not immuned, but just an healthy person
         ...
         if a < qs        %isolation check
            ...
            if num3 ~= 0 % person is vaccinated but not immuned
                c = rand();
                if vaccination_times(num3) > tsec
                if rand < p
                    newly_infected = newly_infected + 1;
